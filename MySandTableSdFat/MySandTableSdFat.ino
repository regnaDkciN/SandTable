...
//   - Moved (most) shape files to SD card.
// - 28-AUG-2026 JMC
...
#include "SdShapes.h"           // For SdShapes class.
...
const int LIGHTS_PIN         = 11;          // Lights (LEDs) output pin.
...
const int SD_CS_PIN          = 39;          // SD card chip select pin.
...
// SD card related data.  NOTE: USE SdFs and FsFile
static const char *SD_ROOT_DIR = "/";   // Shape files root directory.
static SdShapes gSdShapes;              // The shapes class instance.
...
// PlotSdShape()
//
// Plots a shape based on an SD shape file values.
//
// Arguments:
//   - f      : A reference to an open SD shape file.
//   - pName  : Name string of the shape file to plot.
/////////////////////////////////////////////////////////////////////////////////
void PlotSdShape(FsFile &f, const char *pName)
{
    // Show our call.
    StartShape("PlotSdShape(%s)\n", pName);

    // Read from the file until there's nothing else in it:
    Coordinate val(0, 0);

    uint32_t i = 0; // Step counter.
    while (!AbortShape && f.available())
    {
        f.read(&val, sizeof(Coordinate));
        GotoXY((float_t)val.x, (float_t)val.y);
        ...
        if (LOG_STEP)
        {
            ...
            while (!AbortShape)
            {
                ...
                else if (StepPrev)
                {
                    ...
                    {
                        i--;
                        f.seekCur(-2 * (int64_t)sizeof(Coordinate));
                        break;
                    }
                }
            } // End while(!AbortShape)
        } // End LOG_STEP
    } // End while
    EndShape();
}// End PlotSdShape().
...
void RandomSdPlot()
{
    // Select a random file and execute it.
    if (gSdShapes.RandomShapeFile())
    {
        PlotSdShape(gSdShapes.CurFile(), gSdShapes.CurFileName());
    }
    else
    {
        LOG_F(LOG_ALWAYS, "!!! Couldn't open random SD file.\n");
    }
} // End RandomSdPlot().
...
ShapeInfo RandomShapes[] =
{
    ...
    ShapeInfo("RandomSdPlot", RandomSdPlot, 2),
...
void setup()
{
    ...
    // Initialize our SD and its shape files.
    const uint32_t SD_RETRIES = 3;
    for (uint32_t i = 0;
         !gSdShapes.Begin(NextRandom, SD_ROOT_DIR, SD_CS_PIN, DEDICATED_SPI,
                          SPI_FULL_SPEED, &SPI1) && (i < SD_RETRIES); i++)
    {
        delay(1000); // Wait for a second before retrying
    }
    if (!gSdShapes.IsRootOpen())
    {
        Serial.print("Cannot open SD root.\n");
    }
